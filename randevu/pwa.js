(()=>{
  ...
  async function getMessaging(){
    if(!messagingPromise){
      messagingPromise=(async()=>{
        ...
        return {messagingModule:messaging,firebaseApp};
      })();
    }
  ...
  async function registerFirebaseToken(){
    ...
    const {messagingModule}=await getMessaging();
    ...
    const token=await messagingModule.getToken(messagingModule.getMessaging(),{
  ...
  async function saveFcmToken(token,type='customer',bookingId=''){
    try{
      const response=await fetch(API_URL,{
        ...
        body:JSON.stringify({action:'save_fcm_token',token:token,type:type,bookingId:String(bookingId||'')})
      });
  ...
  // Sayfa açılırken service worker hazır olsun; bildirim izni otomatik istenmez.
